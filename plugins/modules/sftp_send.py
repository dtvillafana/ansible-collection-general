...
def main():
    ...
    if os.path.isfile(src):
        try:
            ...
            module.fail_json(msg=f"Unable to read source file: {to_native(e)}", **result)
    else:
        ...
        try:
            ...
            try:
                ...
                if remote_hash == local_hash:
                    result['msg'] = "File already exists at destination with the same content."
                    module.exit_json(**result)
            except IOError:
            ...
            with sftp.file(module.params['dest_path'], 'wb') as f:
                f.write(content)
            result['changed'] = True
            result['msg'] = f"File uploaded successfully to {to_native(module.params['dest_path'])}"
